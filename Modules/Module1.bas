...
Public Sub printStockOpname()
    ...
    With rpt
        ...
        sql = "SELECT *,DATE_FORMAT(op.tgl_input,'%d-%m-%Y') as dateop,DATE_FORMAT(op.tgl_input,'%H:%i:%s') as timeop,@sf:=((o.box_besar*op.kem_besar)+(o.box_kecil*op.kem_kecil)+op.satuan) as sf,@s:=(@sf-stok_sblm) as sisa,(@s*o.harga_beli) as total,p.nm_pengguna "

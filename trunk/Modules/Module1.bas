...
Public Sub cetak_Transfer()
    ...
    On Error GoTo opps
     With Printer
        ...
        .CurrentX = .CurrentX + 500 ' Skip some space
        Printer.Print " Laba Bersih "; Spc(0); ":"; Spc(5); "" & Format(tbl.TABLE_LABA_BERSIH, "##,###0.00") & ""; Tab(70); ""; Spc(3); ""; Spc(2); ""
        .CurrentX = .CurrentX + 500 ' Skip some space
        Printer.Print " Transfer  "; Spc(3); ":"; Spc(5); "" & Format(tbl.TABLE_TRANSFER, "##,###0.00") & ""; Tab(70); " "; Spc(5); ""; Spc(2); ""
        .CurrentX = .CurrentX + 500 ' Skip some space
        Printer.Print " Sisa Kas"; Spc(4); ":"; Spc(5); "" & Format(tbl.TABLE_KAS_SISA, "##,###0.00") & ""; Tab(70); ""; Spc(8); ""; Spc(9); " "
        Printer.Print ""
...
Public Sub cetak_Faktur2()
    ...
    rentang = 9
    On Error GoTo opps
     'If (tbl.TABLE_PAY_TYPE = "Lunas") Then
     With Printer
        .Font.Name = "Arial Narrow"
        .Font.Size = 7
        .CurrentY = .CurrentY + 0 ' Skip some space
    ...
    On Error GoTo opps
     ...
     With Printer
        ...
        If tbl.TABLE_TYPE = "Credit" Then
            ...
            sql = "SELECT k.plafon,(SELECT SUM(j.piutang) FROM tbl_jual j WHERE j.id_kreditor=k.id_kreditor AND j.flag_kreditor=0 AND j.piutang > 0) as limitHutang,(k.plafon-(SELECT SUM(j.piutang) FROM tbl_jual j WHERE j.id_kreditor=k.id_kreditor AND j.flag_kreditor=0 AND j.piutang > 0)) as sisa FROM tbl_kreditor k WHERE k.id_kreditor=" & tbl.TABLE_ID_KREDITUR & " AND (SELECT SUM(j.piutang) FROM tbl_jual j WHERE j.id_kreditor=k.id_kreditor AND j.flag_kreditor=0 AND j.piutang > 0)>k.plafon  "
...
Public Sub cetak_FakturBeli()
    ...
    On Error GoTo opps
     With Printer
        ...
        '.CurrentX = .CurrentX + 500 ' Skip some space
        'Printer.Print " Nama Dept  "; Spc(2); ":"; Spc(5); "" & tbl.TABLE_NM_DEPT & ""; Tab(70); "Relasi "; Spc(5); ":"; Spc(2); "" & tbl.TABLE_RELASI & ""
        Printer.Print ""
        .CurrentX = .CurrentX + 500
        Printer.Print " --------------------------------------------------------------------------------------------------------------- "
        .CurrentX = .CurrentX + 500
...
Public Sub printStock()
    ...
    With rpt
        ...
        .DataControl1.ConnectionString = DBPath
         ...
         Dim baw, bak As Long
         Dim gb, gk As String
         baw = Val(frmProduct.lvList.ListItems(1).Text)
         bak = Val(frmProduct.lvList.ListItems(frmProduct.lvList.ListItems.Count).Text)
         If (baw <= bak) Then
            gb = ">="
            gk = "<="
         Else
...
Public Sub printStockMin()
    ...
    With rpt
        ...
        .DataControl1.CursorLocation = ddADOUseClient
        .DataControl1.ConnectionString = DBPath
        sql = "select *,(stok_min-sisa) as permintaan from vw_stok_min order by id_kategori ASC "
        .DataControl1.Source = sql
        .GroupHeader1.DataField = "id_kategori"
        .txtCategoryName.DataField = "nm_kategori"
        .txtKodeProduct.DataField = "kd_obat"
...
Public Sub printCashFlow()
    ...
    Dim gb, gk As String
    DBPath = "DSN=pos_db"
    baw = Val(frmCashFlow.lvList.ListItems(1).Text)
    bak = Val(frmCashFlow.lvList.ListItems(frmCashFlow.lvList.ListItems.Count).Text)
    If (baw <= bak) Then
        gb = ">="
        gk = "<="
    Else
    ...
    With rpt
        ...
        .DataControl1.CursorLocation = ddADOUseClient
        .DataControl1.ConnectionString = DBPath
        sql = "SELECT tgl_cash,(jual+jual_sebelumnya) as jual,(beli+beli_sebelumnya) as beli FROM vw_cash_flow WHERE flag=0 AND id " & gb & " " & baw & " AND  id  " & gk & " " & bak & " ORDER BY tgl_cash ASC "
        'MsgBox sql
        .DataControl1.Source = sql
...
Public Sub printCashFlowdetails()
    ...
    If (baw <= bak) Then
        gb = ">="
        gk = "<="
        tbl.TABLE_TANGGAL_AWAL = Format(frmCashFlow.lvList.ListItems(1).SubItems(1), "YYYY-MM-DD")
        tbl.TABLE_TANGGAL_AKHIR = Format(frmCashFlow.lvList.ListItems(frmCashFlow.lvList.ListItems.Count).SubItems(1), "YYYY-MM-DD")
    Else
        gb = "<="
        gk = ">="
        tbl.TABLE_TANGGAL_AKHIR = Format(frmCashFlow.lvList.ListItems(1).SubItems(1), "YYYY-MM-DD")
        tbl.TABLE_TANGGAL_AWAL = Format(frmCashFlow.lvList.ListItems(frmCashFlow.lvList.ListItems.Count).SubItems(1), "YYYY-MM-DD")
    End If
...
Public Sub printSalesCommision()
    ...
    With rpt
        ...
        .DataControl1.ConnectionString = DBPath
         Dim baw, bak As Long
         Dim gb, gk As String
         baw = Val(frmKomisi.lvList.ListItems(1).Text)
         bak = Val(frmKomisi.lvList.ListItems(frmKomisi.lvList.ListItems.Count).Text)
         
         If (baw <= bak) Then
            gb = ">="
            gk = "<="
            tbl.TABLE_TANGGAL_AWAL = Format(frmKomisi.lvList.ListItems(1).SubItems(3), "DD/MM/YYYY")
            tbl.TABLE_TANGGAL_AKHIR = Format(frmKomisi.lvList.ListItems(frmKomisi.lvList.ListItems.Count).SubItems(3), "DD/MM/YYYY")
         Else
            gb = "<="
            gk = ">="
            tbl.TABLE_TANGGAL_AWAL = Format(frmKomisi.lvList.ListItems(frmKomisi.lvList.ListItems.Count).SubItems(3), "DD/MM/YYYY")
            tbl.TABLE_TANGGAL_AKHIR = Format(frmKomisi.lvList.ListItems(1).SubItems(3), "DD/MM/YYYY")
        End If
        
        .lblTanggal.Caption = "Dari Tanggal " & tbl.TABLE_TANGGAL_AWAL & " Sampai " & tbl.TABLE_TANGGAL_AKHIR
        'AND j.tgl_jual < curdate()
        'DATE_FORMAT(tgl_jual,'%Y-%m-%d') as
        sql = "SELECT j.no_jual,j.tgl_jual,(IF(j.flag_kreditor=0,(IF (j.id_kreditor>0,(IF(DATE_ADD(DATE_FORMAT(j.tgl_jual,'%Y-%m-%d'),INTERVAL + j.jw DAY)>CURDATE(),'Piutang','Jatuh Tempo')),'Lunas')) ,'Lunas'))AS status,j.tgl_komisi,j.kd_pasien,p.nm_pasien,k.nm_kreditor,d.kd_departement,d.nm_departement,c.nm_cabang,j.flag_kreditor,j.flag_debitor,j.bayar,j.piutang,j.komisi,(j.bayar-j.komisi) as total FROM tbl_jual j JOIN tbl_pasien p ON p.kd_pasien=j.kd_pasien LEFT JOIN tbl_kreditor k ON k.id_kreditor=j.id_kreditor JOIN tbl_departement d ON d.id_departement=j.id_departement LEFT JOIN tbl_cabang c ON c.id_cabang=j.id_cabang LEFT OUTER JOIN tbl_pengguna pp ON pp.id=j.id_pengguna WHERE j.flag_kreditor=1 AND j.id_jual " & gb & " " & baw & " And j.id_jual " & gk & " " & bak & "  ORDER BY j.tgl_komisi ASC,j.no_jual ASC,j.kd_pasien ASC " 'DATE_FORMAT(j.tgl_jual,'%Y-%m-%d') ASC
        'MsgBox sql
        .DataControl1.Source = sql
        '.GroupHeader1.DataField = "tgl_jual"
...
Public Sub printRetur()
    ...
    With rpt
        ...
        .ADO1.ConnectionString = DBPath
         ...
         Dim baw, bak As Long
         Dim gb, gk As String
         baw = Val(frmReturPurchase.lvList.ListItems(1).Text)
         bak = Val(frmReturPurchase.lvList.ListItems(frmReturPurchase.lvList.ListItems.Count).Text)
         If (baw <= bak) Then
            gb = ">="
            gk = "<="
         Else
...
Public Sub printInvoice()
    ...
    With rpt
        ...
        sql = sql + " WHERE j.id_kreditor= " & tbl.TABLE_ID_KREDITUR & " AND j.flag_kreditor=trim(0) AND DATE_FORMAT(j.tgl_jual,'%Y-%m-%d')< CURDATE() "
...
Public Sub printInvoicePembelian()
    ...
    With rpt
        ...
        sql = sql + " WHERE b.flag_supplier=0 AND b.id_supplier = " & Trim(tbl.TABLE_ID_SUPPLIER) & " "
     ...
     With frmPurchaseFaktur
        ...
        If (total > 0) Then
            ...
            For i = 1 To total
                ...
                sql = sql + " flag_supplier= 1 , "
                sql = sql + " bayar=" & Format(.lvList.ListItems(i).SubItems(10), "") & ", "
                'sql = sql + " hutang=" & Format(.lvList.ListItems(i).SubItems(10), "") & " "
                sql = sql + " hutang=0"
                sql = sql + " WHERE no_beli='" & .lvList.ListItems(i).SubItems(1) & "'"
                bayar = bayar + Val(Format(.lvList.ListItems(i).SubItems(10), ""))
                CN.Execute sql
            Next i
            tbl.TABLE_TANGGAL_AWAL = .lvList.ListItems(total).SubItems(2)
            tbl.TABLE_TANGGAL_AKHIR = .lvList.ListItems(1).SubItems(2)
...
Public Sub cetak_Invoice_Pembelian()
    ...
    On Error GoTo opps
     With Printer
        ...
        .CurrentX = .CurrentX + 500 ' Skip some space
        Printer.Print " Total Obat"; Spc(10); ":"; Spc(5); "" & tbl.TABLE_TOTAL_OBAT & ""; Tab(70); ""; Spc(8); ""; Spc(9); " "
        .CurrentX = .CurrentX + 500 ' Skip some space
        Printer.Print " Total Uang"; Spc(9); ":"; Spc(5); "" & tbl.TABLE_TOTAL & ""; Tab(70); ""; Spc(8); ""; Spc(9); " "
        Printer.Print ""
        .CurrentX = .CurrentX + 500
        Printer.Print " --------------------------------------------------------------------------------------------------------------- "
        .EndDoc
...
Public Sub printKomisi()
    ...
    With rpt
        ...
        sql = sql + " WHERE j.id_departement= " & tbl.TABLE_ID_DEPT & " AND j.flag_kreditor=1 AND j.flag_debitor=0  AND DATE_FORMAT(j.tgl_jual,'%Y-%m-%d')<= CURDATE() "
     ...
     'MDIMainMenu.ShowTBButton "fffffft"
End Sub

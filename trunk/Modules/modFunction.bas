...
'Function that return true if the control is empty
Public Function is_zero(ByRef sText As Variant, Optional UseTagValue As Boolean) As Boolean
    On Error Resume Next
    If sText.Text = 0 Then
        is_zero = True
        If UseTagValue = True Then
            MsgBox "The field  '" & sText.Tag & "' is Zero.Please check it!", vbExclamation
        Else
            MsgBox "The field is Zero.Please check it!", vbExclamation
        End If
        sText.SetFocus
    Else
        is_zero = False
...
Public Function is_high(ByRef SLow As Double, ByRef SHigh As Double, Optional UseTagValue As Boolean) As Boolean
    On Error Resume Next
    If Val(SLow) > Val(SHigh) Then
       'MsgBox SLow & "-" & SHigh
        is_high = True
        If UseTagValue = True Then
            MsgBox "The field is Higher.Please check it!", vbExclamation
        Else
            MsgBox "The field is Higher.Please check it!", vbExclamation
        End If
    Else
...
Public Function getRecordCount(ByVal srcField As String, ByVal srcTable As String, Optional srcCondition As String, Optional isFormatted As Boolean) As String
    If srcCondition <> "" Then srcCondition = " " & srcCondition
    Dim rs As New Recordset
    rs.CursorLocation = adUseClient
    rs.Open "SELECT COUNT(" & srcField & ") as TCount FROM " & srcTable & srcCondition, CN, adOpenStatic, adLockReadOnly
...
Public Function toMoney(ByVal srcCurr As String) As String
   ...
   ''Format((lblPrice.Caption - ((lblPrice.Caption * txtDisc.Text) / 100)) * txtQty.Text, "##,###0.00")
   toMoney = Format(srcCurr, "#,##0.00")
End Function

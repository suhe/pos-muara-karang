...
Begin VB.Form frmSalesFaktur 
   ...
   End
End
Attribute VB_Name = "frmSalesFaktur"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
Dim CURR_COL As Integer
Dim rsSalesFaktur As New Recordset
...
Private Sub Form_Load()
    ...
    sql2 = "j.flag_kreditor=0 AND j.id_kreditor<>0 AND DATE_FORMAT(j.tgl_jual,'%Y-%m-%d')< DATE_FORMAT(curdate(),'%Y-%m-%d') "
    If (tbl.TABLE_SEARCH_KREDITOR <> "") Then
        sql2 = sql2 & " AND k.nm_kreditor LIKE '%" & tbl.TABLE_SEARCH_KREDITOR & "%'"
    End If
    
    If (tbl.TABLE_SEARCH_TANGGAL <> "") Then
        sql2 = sql2 & " AND " & tbl.TABLE_SEARCH_TANGGAL
    End If
    
    'MsgBox sql2
    
    With SQLParser
            .Fields = "j.id_jual,j.no_jual,j.tgl_jual,j.flag_kreditor,j.flag_debitor,j.kd_pasien,p.nm_pasien,p.no_tlp,p.relasi,j.id_kreditor,k.nm_kreditor,d.kd_departement,d.nm_departement,c.nm_cabang,j.type,j.payment,j.bayar,j.piutang,(j.bayar-j.piutang) as sisa,j.komisi,((j.bayar-j.piutang)-j.komisi) as laba,pp.nm_pengguna "
    ...
    If rsSalesFaktur.State = 1 Then rsSalesFaktur.Close
    rsSalesFaktur.CursorLocation = adUseClient
    rsSalesFaktur.Open SQLParser.SQLStatement, CN, adOpenStatic, adLockReadOnly
    
    With RecordPage
        .Start rsSalesFaktur, 10000000
        FillList 1
    End With
...
Private Sub FillList(ByVal whichPage As Long)
    ...
    Me.Enabled = False
    Call pageFillListView(lvList, rsSalesFaktur, RecordPage.PageStart, RecordPage.PageEnd, 22, 2, False, True, , , , "id_jual")
    Me.Enabled = True
